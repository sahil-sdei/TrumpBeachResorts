...
/**
 * Created by Gagandeep Singh on 11/09/17.
 */


public abstract class InfiniteAdapterG<B extends ViewDataBinding> extends RecyclerView.Adapter
{
    ...
    {
        return new LoadingViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.progress_dialog, parent, false));
    }
    ...
    {
        B binder = DataBindingUtil.inflate(LayoutInflater.from(parent.getContext())
                , getInflateLayout(), parent, false);
    ...
    /**
     * ViewHolder class for the adapter,We are using binding here ,so a single ViewHolder can be used for all adapters.
     */
